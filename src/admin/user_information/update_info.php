...
$update_stmt = "UPDATE `user_info` SET `birth_date`='$birth_date', `birth_place`='$birth_place', `contact_number`='$contact_number',
 ...
 if ($connect->query($update_stmt) === true) {
    if ($spouse_first_name === "" && $spouse_middle_name === "" && $spouse_last_name === "" && $occupation === "" && $employer === "" && $business_address === "") {
        $update_info = "UPDATE `user_background` SET (`user_id`,`father_first_name`,`father_middle_name`,`father_last_name`,
        ...
        $update_info = "UPDATE `user_background` SET (`user_id`,`spouse_first_name`,`spouse_middle_name`,`spouse_last_name`,
     ...
     if ($connect->query($update_info) === true) {
        $update_info = "UPDATE `user_educ` SET (`user_id`,`elementary`,`secondary`,`college`,`post_grad`) VALUES ('$id','$elementary','$secondary','$college','$post_grad');";
         if ($connect->query($update_info) === true) {
         } else {
         ...
         for ($i=0; $i < count($child_name) ; $i++) {
             if ($child_name[$i] === "" && $child_birth[$i] === "") {
                ...
                $update_info = "UPDATE `user_offspring` SET (`child_name`,`child_birth_date`,`user_id`) VALUES ('$child_name[$i]','$child_birth[$i]','$id');";
                 if ($connect->query($update_info) === true) {
                 } else {
                     print_r($connect->error);
                 }
         ...
         $update_info = "UPDATE `emergency_info_sheet`SET (`user_id`,`coordinates`,`main_address`,`secondary_address`,
         ...
         if ($connect->query($update_info) === true) {
            for ($i=0; $i < count($r_name); $i++) {
                if ($r_name[$i] !== "" ) {
                    $update_info = "UPDATE `relatives` SET (`r_id`,`r_name`,`r_number`,`r_relationship`) VALUES ('$id','$r_name[$i]','$r_mobile_number[$i]','$r_relationship[$i]');";
                    if ($connect->query($update_info) === true) {
                    } else {
                        print_r($connect->error);
                    }
            ...
            for ($i=0; $i < count($h_name); $i++) {
                if ($h_name[$i] !== "" ) {
                    $update_info = "UPDATE `housemates` SET (`h_id`,`h_name`,`h_number`,`h_relationship`) VALUES ('$id','$h_name[$i]','$h_mobile_number[$i]','$h_relationship[$i]');";
                    if ($connect->query($update_info) === true) {
                    } else {
                        print_r($connect->error);
                    }
